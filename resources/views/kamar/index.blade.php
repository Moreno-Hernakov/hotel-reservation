    ...
    <div class="row">
        <div class="col-lg-6 mb-2">
            <div class="card border-0 shadow">
                <div class="card-header bg-primary fs-4 fw-bold text-light">Daftar Tipe Kamar 
                    <button type="button" class="btn btn-info rounded d-inline float-end" data-bs-toggle="modal" data-bs-target="#tipe-kamar">
                        Tambah Tipe
                    </button>
                </div>
                    ...
                    <table class="table table-responsive table-borderless table-hover">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td><a class="btn btn-sm btn-success" data-bs-toggle="modal">Edit</a>
        ...
        <div class="col-lg-6">
            <div class="card border-0 shadow">
                <div class="card-header bg-primary fs-4 fw-bold text-light">Daftar Kamar
                    <button type="button" class="btn btn-info rounded d-inline float-end" data-bs-toggle="modal" data-bs-target="#kamar">
                        Tambah Kamar
                    </button>
                </div>
                <div class="card-body">
                    ...
                    <table class="table table-borderless table-responsive table-hover">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>{{ $kamar->tipe->nama_tipe }}</td>
                                <td>{{ $kamar->status }}</td>
                                <td>
                                    {{-- <a class="btn btn-warning" href="/master-kamar/{{ $kamar->id }}"></a> --}}
                                    <button class="btn btn-sm btn-warning btn-detail open_modal" value="{{$kamar->id}}">Edit</button>

                                    <form class="d-inline" action="{{ route('master-kamar.destroy', $kamar->id) }}" method="post">
                                        @csrf
                                        @method('delete')
                                        <button onclick="return confirm('Yakin ingin menghapus?')" type="submit" class="btn btn-sm btn-danger">Hapus</button>
                                    </form>    
...
<script>
    $(document).on('click','.open_modal',function(){
        var url = "/master-kamar";
        var kamar_id = $(this).val();
        $.get(url + '/' + kamar_id , function (data) {
            //success data
            const target = "{{ url('master-kamar/:id') }}".replace(':id', data.id)
